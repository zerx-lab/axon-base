...
import { extractErrorMessage, extractDetailedError } from "@/lib/error-extractor";
  ...
  try {
    ...
    if (kbIds && kbIds.length > 0 && userQuery) {
      ...
      const customStream = new ReadableStream({
        async start(controller) {
          try {
            ...
            const finalError = capturedError || error;
     ...
     return new Response(
       ...
       { status: extracted.statusCode || 500, headers: { "Content-Type": "application/json" } }
     );
   }
}
